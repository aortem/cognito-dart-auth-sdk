...
import 'package:cognito_dart_auth_sdk/src/cognito_storage.dart';
...
/// The base class to initialize the cognito dart admin sdk
class cognitoApp {
  ///Instance of the cognito App
  static cognitoApp? _instance;
  ...
  ///The class to interact with the various cognito auth methods
  static cognitoAuth? cognitoAuth;
  static GetAccessTokenWithGeneratedToken? _accesstokenGen;
  static GenerateCustomToken? _tokenGen;

  ///The class to interact with the various cognito storage methods
  static cognitoStorage? cognitoStorage;
  ...
  ///Exposed the cognito app singleton
  static cognitoApp get instance {
    if (_instance == null) {
      throw ("cognitoApp is not initialized. Please call initializeApp() first.");
    }
    return _instance!;
  }
  ...
  static Future<cognitoApp> initializeAppWithEnvironmentVariables({
    ...
    // Returns an instance of cognitoApp if it exists or create a new instance based on the parameters passed
    return _instance ??= cognitoApp._(
      apiKey,
      projectId,
      authdomain,
      messagingSenderId,
  ...
  static Future<cognitoApp> initializeAppWithServiceAccount({
    ...
    try {
      ...
      throw Exception('Failed to initialize cognito with service account: $e');
    }
  }
  ...
  static Future<cognitoApp> initializeAppWithServiceAccountImpersonation({
    required String serviceAccountContent,
  ...
  ///Returns a cognito Auth instance associated with the Project
  ///Throws not initialized if cognito app is not intialized
  cognitoAuth getAuth() {
    ...
    return cognitoAuth ??= cognitoAuth(
      apiKey: _apiKey,
  ...
  ///Used to get cognito storage
  ///[apiKey] is the API Key associated with the project
  ///[projectId] is the ID of the project
  cognitoStorage getStorage() {
    assert(_apiKey != null, 'API Key is null');
    assert(_projectId != null, 'Project ID is null');
    if (_instance == null) {
      throw ("cognitoApp is not initialized. Please call initializeApp() first.");
    }
    // Use the getStorage method to obtain a cognitoStorage instance
    return cognitoStorage ??= cognitoStorage.getStorage(
      apiKey: _apiKey!,
      projectId: _projectId!,
      bucketName: _bucketName!,
    );
