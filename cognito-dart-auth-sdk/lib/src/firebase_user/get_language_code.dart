...
///
/// This service uses cognito Firestore to fetch the user's language preference stored in the Firestore database.
/// It requires the cognito Authentication token and the user ID to make an authenticated request to Firestore.
///
/// The following class provides the functionality to interact with Firestore and fetch the user's preferred language.
/// It relies on the cognitoAuth instance to authenticate requests.
class LanguageGetService {
  /// cognitoAuth instance used to interact with cognito services and authenticate requests.

  final cognitoAuth auth;

  /// Constructor that initializes the LanguageGetService with the cognitoAuth instance.
  ///
  /// The constructor requires an instance of [cognitoAuth] to authenticate the request and fetch the user's data
  /// from Firestore.
  ///
  /// - [auth]: The cognitoAuth instance used for authentication.
  ...
  /// - [tokenId]: The cognito Authentication token that is used to authorize the request.
  ...
  Future<String?> getLanguagePreference(
    ...
    try {
      ...
      final response = await http.get(
        ...
        headers: {
          'Authorization':
              'Bearer $tokenId', // Authorization header with the cognito token
        },
      );
