...
/// A service that performs actions before an authentication state change in cognito.
///
/// This service intercepts the authentication state before it's changed, allows for custom actions to be performed,
/// and then makes an HTTP request to the cognito Authentication REST API to refresh the authentication token.
///
...
class cognitoBeforeAuthStateChangeService {
  /// The [cognitoAuth] instance used for authentication operations.
  final cognitoAuth auth;

  /// Constructs an instance of [cognitoBeforeAuthStateChangeService].
  ///
  /// Parameters:
  /// - [auth]: The [cognitoAuth] instance that provides authentication functionality.
  cognitoBeforeAuthStateChangeService(this.auth);
  ...
  /// 3. Makes an HTTP request to cognito Authentication REST API to refresh the authentication token.
  ...
  ///
  /// Throws a [cognitoAuthException] if any step in the process fails.
  Future<void> beforeAuthStateChange(
    ...
    try {
      ...
      // Step 3: Make an HTTP request to cognito Authentication REST API to refresh the auth token
  ...
  /// Refreshes the authentication token using the cognito Authentication REST API.
  ///
  /// This method sends a request to the cognito Authentication service to exchange the provided refresh token
  ...
  ///
  /// Returns a [Future] that resolves to an [http.Response] containing the response from cognito's API.
  ///
  ...
  Future<http.Response> _refreshAuthToken(
    ...
    // Making the HTTP request to cognito Authentication to refresh the token
