...
/// language code from cognito Authentication and possibly set it based on the user's device language.
class UseDeviceLanguageService {
  /// The [cognitoAuth] instance used for performing authentication actions.
  final cognitoAuth auth;

  /// Constructs an instance of [UseDeviceLanguageService].
  ///
  /// Parameters:
  /// - [auth]: The [cognitoAuth] instance that handles cognito Authentication requests.
  UseDeviceLanguageService({required this.auth});

  /// Retrieves the language code set for the user's device.
  ///
  /// This method sends a request to cognito's Authentication API to look up the user's information,
  ...
  ///
  /// Throws:
  /// - [cognitoAuthException] if the operation fails (e.g., network issue or incorrect response).
  Future<String?> useDeviceLanguage(String userId, String languageCode) async {
    try {
      // The URL for the cognito Authentication API endpoint to retrieve user info.
      final url = 'lookup';
      ...
      // Perform the request to cognito Authentication to get user information.
      ...
      // Throw a cognitoAuthException if the operation fails.
