...
class VerifyBeforeEmailUpdate {
  ...
  ///
  /// This method sends a request to cognito to initiate the email change process.
  ...
  ///
  /// Throws:
  /// - [cognitoAuthException] if the request to verify the email fails.
  Future<bool> verifyBeforeEmailUpdate(
    ...
    try {
      ...
      // Perform the request to cognito to verify the email address before updating.
      await auth.performRequest(
        'sendOobCode', // cognito endpoint for sending an out-of-band (OOB) verification code.
        {
          "requestType":
              "VERIFY_AND_CHANGE_EMAIL", // Request type for email verification.
          "idToken": auth.currentUser?.idToken, // The user's cognito ID token.
      ...
      // Handle any errors and throw a cognitoAuthException with an error code.
