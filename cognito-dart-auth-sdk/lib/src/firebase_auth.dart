...
import 'package:cognito_dart_auth_sdk/src/auth/apple_sign_in_auth.dart';
import 'package:cognito_dart_auth_sdk/src/auth/gcp_auth.dart';
import 'package:cognito_dart_auth_sdk/src/cognito_user/link_with_credentails.dart';

import 'package:ds_standard_features/ds_standard_features.dart' as http;
import 'package:cognito_dart_auth_sdk/src/auth/generate_custom_token.dart';
import 'package:cognito_dart_auth_sdk/src/service_account.dart';
import 'package:cognito_dart_auth_sdk/cognito_dart_auth_sdk.dart';
import 'package:cognito_dart_auth_sdk/src/platform_resolver.dart';
import 'package:cognito_dart_auth_sdk/src/auth/apply_action_code.dart';
import 'package:cognito_dart_auth_sdk/src/auth/email_password_auth.dart';
import 'package:cognito_dart_auth_sdk/src/auth/custom_token_auth.dart';
import 'package:cognito_dart_auth_sdk/src/auth/email_link_auth.dart';
import 'package:cognito_dart_auth_sdk/src/auth/get_additional_user_info.dart';
import 'package:cognito_dart_auth_sdk/src/auth/link_provider_to_user.dart';
import 'package:cognito_dart_auth_sdk/src/auth/phone_auth.dart';

import 'package:cognito_dart_auth_sdk/src/auth/reload_user.dart';
import 'package:cognito_dart_auth_sdk/src/auth/send_email_verification_code.dart';
import 'package:cognito_dart_auth_sdk/src/auth/set_language_code.dart';
import 'package:cognito_dart_auth_sdk/src/auth/sign_out_auth.dart';
import 'package:cognito_dart_auth_sdk/src/auth/oauth_auth.dart';
import 'package:cognito_dart_auth_sdk/src/auth/unlink_provider.dart';
import 'package:cognito_dart_auth_sdk/src/auth/update_current_user.dart';
import 'package:cognito_dart_auth_sdk/src/auth/update_password.dart';
import 'package:cognito_dart_auth_sdk/src/auth/update_profile.dart';
import 'package:cognito_dart_auth_sdk/src/auth/user_device_language.dart';
import 'package:cognito_dart_auth_sdk/src/auth/verify_before_email_update.dart';
import 'package:cognito_dart_auth_sdk/src/auth/verify_password_reset_code.dart';
import 'package:cognito_dart_auth_sdk/src/http_response.dart';
import 'package:cognito_dart_auth_sdk/src/popup_redirect_resolver.dart';

// New imports for Sprint 2 #16 to #21
import 'package:cognito_dart_auth_sdk/src/auth/password_reset_email.dart';
import 'package:cognito_dart_auth_sdk/src/auth/revoke_access_token.dart';
...
import 'package:cognito_dart_auth_sdk/src/auth/fetch_sign_in_methods.dart';
import 'package:cognito_dart_auth_sdk/src/auth/create_user_with_email_and_password.dart';
import 'package:cognito_dart_auth_sdk/src/auth/connect_auth_emulator.dart';
import 'package:cognito_dart_auth_sdk/src/auth/get_redirect_result.dart';
import 'package:cognito_dart_auth_sdk/src/auth/recaptcha_config.dart';
import 'package:cognito_dart_auth_sdk/src/auth/confirm_password_reset.dart';
...
import 'cognito_user/get_language_code.dart';

import 'cognito_user/delete_user.dart';

import 'auth/parse_action_code_url.dart';

import 'cognito_user/set_language_code.dart';
import 'id_token_result_model.dart';

///Base cognito auth class that contains all the methods provided by the sdk
class cognitoAuth {
  ///THe api key for cognito project
  final String? apiKey;

  ///The project id for your cognito project
  final String? projectId;

  ///The access token to the cognito project
  ...
  /// The service account of your cognito project
...
////Ticket mo 36 to 41///////////////
  late cognitoPhoneNumberLink cognitoPhoneNumberLink;

  /// Parase cognito url link
  late cognitoParseUrlLink cognitoParseUrlLink;

  /// Delete User
  late cognitoDeleteUser cognitoDeleteUser;
  ...
  ///Sign in anonymously
  late cognitoSignInAnonymously signInAnonymously;
  ...
  /// Get cognito language code
  late LanguageGetService getLanguageService;

  /// cognito before auth change
  late cognitoBeforeAuthStateChangeService cognitoBeforeAuthStateChangeService;

  /// Current cognito auth user
  User? currentUser;
  ...
  cognitoAuth({
    ...
    cognitoPhoneNumberLink = cognitoPhoneNumberLink(this);
    cognitoParseUrlLink = cognitoParseUrlLink(auth: this);
    cognitoDeleteUser = cognitoDeleteUser(auth: this);
    ...
    signInAnonymously = cognitoSignInAnonymously(this);
    ...
    cognitoBeforeAuthStateChangeService =
        cognitoBeforeAuthStateChangeService(this);
  }
  ...
  Future<void> signOut() async {
    log("hekko");
    log("hekko${cognitoApp.instance.getCurrentUser()}");
    if (cognitoApp.instance.getCurrentUser() == null) {
      throw cognitoAuthException(
    ...
    try {
      ...
      cognitoApp.instance.setCurrentUser(null);
      currentUser = null;
      log('User Signout ');
      return;
    } catch (e) {
  ...
  ///Parse a cognito action code URL
  ...
  ///cognitoUser phone number link
  Future<void> cognitoPhoneNumberLinkMethod(
      String phone, String verificationCode) async {
    try {
      await cognitoPhoneNumberLink.linkPhoneNumber(
  ...
  Future<void> deletecognitoUser() async {
    if (cognitoApp.instance.getCurrentUser() == null && currentUser == null) {
      throw cognitoAuthException(
        code: 'user-not-signed-in',
        message: 'No user is currently signed in.',
      );
    }
    // var accesToken = cognitoAppInitialize.instance.getAccessToken();
    //  log("access token is$accesToken");
    try {
      await cognitoDeleteUser.deleteUser(
  ...
  /////////cognitoUser.getIdToken
  Future<String?> getIdToken() async {
    final user = currentUser;
    return await user?.getIdToken();
  }

  //////////// cognitoUser.getIdTokenResult
  Future<IdTokenResult?> getIdTokenResult() async {
    final user = currentUser;
    return await user?.getIdTokenResult();
  }

/////////////////cognito SignIn Anonymously /////////////////////////
...
/////////////////
  ...
  Future<void> getAuthBeforeChange() {
    try {
      return cognitoBeforeAuthStateChangeService.beforeAuthStateChange(
        currentUser!.idToken!,
  ...
  /// Disposes of the cognitoAuth instance and releases resources.
  void dispose() {
